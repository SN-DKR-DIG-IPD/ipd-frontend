...
export class ProcessInstanceService {
  ...
  createProcessInstance(containerId: string, processId: string, data: any = {}): Observable<any> {
    return this.http.post(`${this.apiUrl}containers/${containerId}/processes/${processId}/instances`, data);
  }

  getProcessInstance(containerId: string, processInstanceId: number): Observable<any> {
    return this.http.get(`${this.apiUrl}containers/${containerId}/processes/instances/${processInstanceId}`);
  }

  // ✅ MÉTHODES PROMISE (compatibilité avec l'existant)
  async getAllProcessInstances(containerId: string, headers?: any): Promise<any> {
    try {
      const result = await this.http.get(`${this.apiUrl}containers/${containerId}/processes/instances`).toPromise();
  ...
  async getProcessInstanceVariables(containerId: string, processInstanceId: number, headers?: any): Promise<any> {
    try {
      const result = await this.http.get(`${this.apiUrl}containers/${containerId}/processes/instances/${processInstanceId}/variables`).toPromise();
      return result || {};
    } catch (error: any) {
  ...
  async displayOneProcessInstanceDetail(containerId: string, processInstanceId: number, headers?: any): Promise<any> {
    try {
      const result = await this.http.get(`${this.apiUrl}containers/${containerId}/processes/instances/${processInstanceId}`).toPromise();
      return result;
    } catch (error: any) {
  ...
  async createOneProcessInstance(containerId: string, processId: string, bodyJSON: any = {}, headers?: any): Promise<number> {
    try {
      const result = await this.http.post(`${this.apiUrl}containers/${containerId}/processes/${processId}/instances`, bodyJSON).toPromise();
