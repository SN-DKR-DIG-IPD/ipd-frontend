...
import { ITaskAdapter } from "../itask-adapter";
...
class TaskRestAdapter implements ITaskAdapter {
    ...
    async getConnectedUserTasks(baseUrl: string, headers: HeadersInit, status='status=Created&status=Ready&status=Reserved&status=InProgess&status=Suspended&status=Completedstatus=Reserved&status=Created&status=Failed&status=Error&status=Exited&status=Obsolete' ): Promise<TaskInstances> {
        const response = await fetch(`${baseUrl}server/queries/tasks/instances/owners`, {headers});
		const jsonResp = await response.json();
        return jsonResp;
    }

    async getConnectedUserPotentialTasks(baseUrl: string, headers: HeadersInit, status='status=Created&status=Ready&status=Reserved&status=InProgess&status=Suspended&status=Completedstatus=Reserved&status=Created&status=Failed&status=Error&status=Exited&status=Obsolete' ): Promise<TaskInstances> {
        const response = await fetch(`${baseUrl}server/queries/tasks/instances/pot-owners`, {headers});
		const jsonResp = await response.json();
        return jsonResp;
    }
